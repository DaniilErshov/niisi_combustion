    ...
    for (double koeff_topl = 1; koeff_topl <= 1.6; koeff_topl += 0.5)
    {
        for (Tstart = 300; Tstart < 800; Tstart += 200) {
            ...
            while (N_x < 400)
            {
                ...
                if (number_epoch > 8) add_cell_start = 0;
            ...
            cout << " norm = " << norm[0] << "\n";
            cout << " x_right = " << x_vect[x_vect.size() - 1] << "\n";
            Write_to_file("D:/Storage/Daniil/gorenie_try1/0709_gorenie/chemkin_heptane/KINSOL_" + to_string(koeff_topl) + " " + to_string(Tstart), fout, x_vect,
                T_vect, Y_vect, Y_vect, M, N_x, 1);
